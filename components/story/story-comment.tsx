...
export function StoryReply({
  ...
  useEffect(() => {
    if (!currentUser || !storyOwnerId) return;
    ...
    const q = query(
      ...
      where("contactId", "==", storyOwnerId)
    );

    const unsubscribe = onSnapshot(q, (snapshot) => {
      setIsContact(!snapshot.empty);
      setIsLoadingContact(false);
    });

    return () => unsubscribe();
  }, [currentUser, storyOwnerId, db]);
  ...
  const isDisabled =
    !isInitialized || isSending || !isContact || isLoadingContact;

  return (
    <div className="w-full flex flex-col items-center gap-2 relative">
      {showEmoji && (
        <div className="absolute bottom-16 z-50 animate-in slide-in-from-bottom-5 origin-bottom-left left-1/2 -translate-x-[45vw] max-w-[95vw] overflow-hidden md:left-0 md:translate-x-0 md:max-w-none md:overflow-visible">
          <Picker
            data={data}
      ...
      <div className="flex w-full items-end gap-2">
        <div className="flex-1 flex items-end gap-1 bg-black/40 backdrop-blur-md border border-white/20 rounded-[24px] px-2 py-1.5 focus-within:border-white/50 focus-within:bg-black/60 transition-colors">
          ...
          <Button
            size="icon"
            className={`h-8 w-8 shrink-0 rounded-full transition-all duration-200 bg-primary text-primary-foreground hover:bg-primary/90 mb-0.5`}
            onClick={() => {
